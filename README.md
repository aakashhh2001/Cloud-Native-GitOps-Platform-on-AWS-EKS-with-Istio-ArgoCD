...
Developer Push: Code is committed to the main branch of the GitHub repository.

CI Pipeline (GitHub Actions): * Authenticates securely with AWS via OIDC.
...
CD Pipeline (ArgoCD): * Detects the drift in the GitHub repository.

Automatically synchronizes the AWS EKS cluster to match the desired state defined in Git.

Traffic Management: Istio Ingress Gateway intercepts traffic from the AWS Application Load Balancer and routes it to the Python application pods via VirtualServices.

Notification: AWS SES sends an automated email (ramanan11032026@gmail.com) confirming the successful deployment.
